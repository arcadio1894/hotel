...
@section('page-header')
    <h3 class="m-0 text-dark">Editar Reserva</h3>
@endsection
...
@section('page-title')
    <div class="col-10">
        <h5 class="card-title col-7">Actualizar la reserva</h5>
    </div>
@endsection
...
@section('content')
    ...
    <form class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input class="form-control" id="document" type="text" value="{{$reserva->customer->document}}" readonly>
        </div>
        <div class="col-md-6">
            <label>&nbsp;</label><br>
    ...
    <form class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select name="reservationType" id="reservationType" class="form-select" disabled>
                <option value="0" {{ $reserva->reservationType == 0 ? 'selected' : '' }}>Selecciona</option>
                <option value="1" {{ $reserva->reservationType == 1 ? 'selected' : '' }}>Por Hora</option>
                <option value="2" {{ $reserva->reservationType == 2 ? 'selected' : '' }}>Por Día</option>
            </select>
        </div>

        <!-- Campos para "Reserva por horas" -->
        @if($reserva->reservationType == 1)
        <!-- Campos para "Reserva por horas" -->
            <div class="row" id="hourFields">
                <div class="col-md-4">
                    <label for="selectedDate">Fecha: <span class="text-danger">*</span></label>
                    <input class="form-control" type="date" id="selectedDate" value="{{ $startDate->format('Y-m-d') }}">
                </div>
                <div class="col-md-4">
                    <label for="selectedStartTime">Hora de inicio: <span class="text-danger">*</span></label>
                    <input class="form-control" type="time" id="selectedStartTime" value="{{ $reserva->start_date->format('H:i') }}">
                </div>
                <div class="col-md-4">
                    <label for="hoursQuantity">Cantidad de Horas <span class="text-danger">*</span></label>
                    <input class="form-control" type="number" id="hoursQuantity" value="{{ $reserva->end_date->format('H:i') }}">
                </div>
            </div>
        @elseif($reserva->reservationType == 2)
        <!-- Campos para "Reserva por días" -->
            <div class="row" id="dayFields">
                <div class="col-md-4">
                    <label for="startDate">Fecha de Inicio: <span class="text-danger">*</span></label>
                    <input class="form-control" type="date" id="startDate" value="{{ $startDate->format('Y-m-d') }}">
                </div>
                <div class="col-md-4">
                    <label for="endDate">Fecha de Fin: <span class="text-danger">*</span></label>
                    <input class="form-control" type="date" id="endDate" value="{{ $endDate->format('Y-m-d') }}">
                </div>
                <div class="col-md-4">
                    <label for="startTime">Hora de inicio: <span class="text-danger">*</span></label>
                    <input class="form-control" type="time" id="startTime" value="{{ $reserva->start_date->format('H:i') }}">
                </div>
            </div>
        @endif
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <input class="form-control" id="total_guest" name="total_guest" type="number" placeholder="2" value="{{$reserva->total_guest}}" autocomplete="off">
            </div>
        ...
        <div class="col-md-3">
            ...
            <select class="form-select" id="paymethod" name="paymethod">
                ...
                @foreach($paymethods as $paymethod)
                    <option value="{{$paymethod->id}}" @if($paymethod->id == $reserva->paymethod_id) selected @endif>{{$paymethod->name}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-3">
            ...
            <input class="form-control" id="initialpay" type="number" placeholder="100"  value="{{$reserva->initial_pay}}">
        </div>
        <hr>
